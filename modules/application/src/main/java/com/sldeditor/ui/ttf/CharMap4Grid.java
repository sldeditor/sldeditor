...
MouseMotionListener, MouseWheelListener
{
        ...
        case (MouseWheelEvent.WHEEL_BLOCK_SCROLL):
        {
            charMap4.getGridScroll().setValue(charMap4.getGridScroll().getValue()
                + (event.getWheelRotation()
                                * charMap4.getGridScroll().getBlockIncrement()));
        }
        break;
    ...
    protected void paintComponent(Graphics context)
    {
        ...
        else if (charMap4.getDisplayFont().equals(panelFont) == false) // a new font?
        {
            ...
            charMap4.setStatusText(CharMap4.getFormatComma().format(charCount)
                + " characters with " + CharMap4.getFormatComma().format(glyphCount)
    ...
    void paintGridCell(Graphics2D gr2d, FontRenderContext render, int index)
    {
        ...
        if((index == selectedChar) && (selectedFont.compareTo(charMap4.getDisplayFont().getFontName()) == 0))
        {
