 ...
 */
public class MultiLayerMarkerSymbol extends BaseSymbol implements EsriSymbolInterface,
EsriMarkSymbolInterface, EsriFillSymbolInterface {
    ...
    /* (non-Javadoc)
     * @see com.sldeditor.convert.esri.symbols.EsriMarkSymbolInterface#convert(com.google.gson.JsonElement)
     */
    @Override
    ...
    public void convert(Rule rule, JsonElement element, String layerName, int transparency) {
        ...
        if(layerArray.size() > 0)
        {
            for(int index = 0; index < layerArray.size(); index ++)
            {
                ...
                List<Graphic> markerList = SymbolManager.getInstance().getMarkerList(obj.get(MultiLayerMarkerSymbolKeys.MARKER).getAsJsonObject());
